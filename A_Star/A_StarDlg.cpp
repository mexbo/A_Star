...
int CallBack(void *p)//非CA_StarDlg内部函数
{
	if (p == NULL)
		return 0;
...
BOOL CA_StarDlg::OnInitDialog()
{
	...
	m_mapinfo.nRow = 15;//行
	m_mapinfo.nColumn = 15;//列

	m_deltaX = m_nWidth / m_mapinfo.nColumn;//每一小格子的宽
	m_deltaY = m_nHeight / m_mapinfo.nRow;//每一小格子的高
	...
	{
		m_mapinfo.vecBlock.push_back(node[i]);//最开始添加障碍物
	}
...
void CA_StarDlg::DrawSquare(COLORREF &brush, const NODE *node)//画格子
{
	RECT rect;
	CBrush br(brush);
...
void CA_StarDlg::OnClickedButton()
{
	...
	m_AStar.SetRange(m_mapinfo.nRow, m_mapinfo.nColumn);//设置范围
	m_AStar.SetStartAndEnd(m_mapinfo.start, m_mapinfo.end);//设置开始和结束位置
	m_AStar.SetBlock(m_mapinfo.vecBlock);//设置障碍物
...
void CA_StarDlg::OnClickedPicture()
{
	...
	CPoint point;
	GetCursorPos(&point);//光标坐标
	int x, y;
	PointToCoord(point, x, y);//转换为格子坐标
	SetSquareProperty(x, y);//设置格子的属性
}
